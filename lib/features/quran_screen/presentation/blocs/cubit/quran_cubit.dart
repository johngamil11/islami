...
class QuranCubit extends Cubit<List<String>> {
  ...
  List<String> surahNames = []; // تخزين أسماء السور بشكل منفصل
  List<String> verses = []; // تخزين الآيات بدون التأثير على أسماء السور
  ...
  void loadSurahName() {
    ...
    emit(surahNames); // تحديث القائمة دون التأثير على المحتوى
  }

  void loadSurahContent(int index) async {
    verses = await repository.getSurahContent(index);
    emit(verses); // تحديث الحالة بالآيات عند فتح سورة معينة
  }

  void resetToSurahNames() {
    emit(surahNames); // استرجاع أسماء السور عند الرجوع
  }
}
